...
class AccountWHTCertificate(models.Model):
    ...
    _sql_constraints = [
        ...
        ('certificate_business_unique', 
         ...
         'UNIQUE(company_id, partner_id, wht_type, income_type, pay_date, custom_key)',
    ...
    def _generate_custom_key(self):
        ...
        NEW FORMAT (Production-Safe & Deterministic):

        "{payments_str}_{bills_str}_{wht_type}_{income_type}_{payment_date}"

        

        Components:

        - payments_str: String of all payment IDs (supports grouped multi-payment batches)

        - bills_str: String of related bill IDs (stable hash for grouped certificates)
        ...
        Returns:
            ...
            str: Deterministic Idempotency key for business-level uniqueness
        ...
        # Component 1: Payments string (supports multiple payments in grouped flow)
        ...
        payments_str = "-".join(map(str, pmt_ids_sorted)) if pmt_ids_sorted else 'manual'

        

        # Component 2: Bills string (stable format for grouped certificates)

        bill_ids = self.move_ids.mapped('id') if self.move_ids else []

        if not bill_ids and self.move_id:

            bill_ids = [self.move_id.id]

        bill_ids_sorted = sorted(bill_ids)

        # Using self.id for fallback if no move_ids exist. 'draft' for NewId

        record_id = str(self.id) if isinstance(self.id, int) else 'draft'

        bills_str = "-".join(map(str, bill_ids_sorted)) if bill_ids_sorted else record_id

        

        # Component 3: WHT Type

        wht_type = self.wht_type or 'none'

        

        # Component 4: Income Type

        income_type = self.income_type or 'none'

        

        # Component 5: Payment Date
        ...
        # Generate composite key (MUST MATCH SQL MIGRATION LOGIC)

        custom_key = f"{payments_str}_{bills_str}_{wht_type}_{income_type}_{payment_date_str}"
    ...
    def get_partner_address_inline(self):
        """
        Return partner address as a single clean line for WHT PDF reports.

        Uses the stored partner_address field (snapshot data) and normalizes
        all whitespace characters (\\n, \\r, \\t, multiple spaces) into a single space.

        This is a presentation-layer helper — it does NOT modify stored data.
        Safe for use in QWeb reports (50 ทวิ, eTax, etc.)
        """
        self.ensure_one()
        raw = self.partner_address or ''
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            if rec.state == 'done' and not self.env.context.get('allow_historical_edit'):
                ...
                protected_fields = ['certificate_no', 'move_ids', 'payment_id', 'payment_ids', 'custom_key', 'wht_type', 'wht_pay_type', 'partner_name_snapshot', 'bill_reference_snapshot', 'branch_snapshot', 'invoice_date_snapshot']
...
class AccountWHT(models.Model):
    ...
    type_tax_use = fields.Selection(
        [
            ...
            ("purchase", "Purchases"),
            ("none", "None"),
            ("tax", "[DEPRECATED] Tax on Tax"),
        ],
        string="Tax Type",
        default="sale",
        required=True,
        help="DEPRECATED: Use calculation_base='vat' instead. This option is kept temporarily for backward compatibility.",
    )

    def is_vat_based_wht(self):
        self.ensure_one()
        return (
            self.calculation_base == 'vat'
            or self.type_tax_use == 'tax'
        )
